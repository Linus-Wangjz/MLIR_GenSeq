...
void AffineLoadVectorize::runOnOperation() {
  ...
  std::vector<mlir::affine::AffineLoadOp> loads;
  ...
  for (mlir::affine::AffineLoadOp ld : loads) {
    ...
    mlir::AffineMap map = ld.getAffineMap();
    ...
    for (unsigned i = 0, e = map.getNumResults(); i < e; ++i) {
      ...
      Value effectiveIndex = mlir::affine::makeComposedAffineApply(
    ...
    auto vload = rewriter.create<mlir::vector::LoadOp>(
