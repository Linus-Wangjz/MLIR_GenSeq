...
4. Lowering from other dialect to LLVM dialect has to be done with "polygeist-opt" instead of "mlir-opt". Run "{$POLYGEIST_BUILD_DIR}/bin/polygeist-opt ./benchmark/NW.mlir --lower-affine --convert-polygeist-to-llvm > ./benchmark/NW_llvm.mlir" to convert to llvm dialect. Notice the "--lower-affine" option is needed as "--convert-polygeist-to-llvm" option assumes that affine has already be lowered.

5. Use the mlir-translatr with option "--mlir-to-llvmir" to translate LLVM dialect to LLVM IR. Run "{$LLVM_BUILD_DIR}/bin/mlir-translate --mlir-to-llvmir ./benchmark/NW_llvm.mlir -o NW.ll"

6. Use clang to compile the LLVM IR. Notice Step 5 uses opaque pointer as default, so your LLVM version should be newer than 15